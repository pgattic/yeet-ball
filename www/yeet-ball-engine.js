...
function touchHandler(e) {
	...
	for(var i=0; i < e.changedTouches.length; i++) {
		...
		var touchY = e.changedTouches[i].pageY;
//		if (!onMenu) {
			if (touchX >= pauseX && touchX <= pauseX + pauseWidth && touchY >= pauseY && touchY <= pauseY + pauseWidth) {
				isPaused = !isPaused;
			}
			if (!isPaused) {
				if (touchX >= superX && touchX <= superX + superWidth && touchY >= superY && touchY <= superY + superWidth) {
					if (powerUpsLeft > 0) {
						superTimer = 250;
						powerUpsLeft--;
...
//		}
//		else {
			//CODE FOR THE MENU INTERACTIONS
//		}
	}
}
...
function drawMiniMap() {
	ctx.beginPath();
	ctx.rect(((canvas.width / 2) - screenWidth / 8) + player.x, ((-canvas.height / 2) + screenHeight / 40) + player.y, screenWidth / 10, screenHeight / 10);
	ctx.fillStyle = bgColor;
	ctx.fill();
	ctx.lineWidth = 1;
	ctx.strokeStyle = lineColor;
	...
	for (let i = 1; i < 8; i++) {
		ctx.beginPath();
		ctx.moveTo(((canvas.width / 2) - screenWidth / 8) + player.x + (screenWidth / 80) * i, ((-canvas.height / 2) + screenHeight / 40) + player.y);
		ctx.lineTo(((canvas.width / 2) - screenWidth / 8) + player.x + (screenWidth / 80) * i, ((-canvas.height / 2) + screenHeight / 40) + player.y + (screenWidth / 10));
		ctx.lineWidth = 1;
		ctx.strokeStyle = gridColor;
		ctx.stroke();

		ctx.beginPath();
		ctx.moveTo(((canvas.width / 2) - screenWidth / 8) + player.x, ((-canvas.height / 2) + screenHeight / 40) + player.y + (screenHeight / 80) * i);
		ctx.lineTo(((canvas.width / 2) - screenWidth / 8) + player.x + (screenWidth / 10), ((-canvas.height / 2) + screenHeight / 40) + player.y + (screenHeight / 80) * i);
		ctx.lineWidth = 1;
		ctx.strokeStyle = gridColor;
		ctx.stroke();
	}
	ctx.beginPath();
	ctx.rect(((((canvas.width / 2) - screenWidth / 8) + player.x * 1.1) - canvas.width / 20), ((((-canvas.height / 2) + screenHeight / 40) + player.y * 1.1) - canvas.height / 20), canvas.width / 10, canvas.height / 10);
	ctx.lineWidth = 1;
	ctx.strokeStyle = miniMapColor;
	ctx.stroke();
	ctx.closePath();
	ctx.beginPath();
	ctx.arc((((canvas.width / 2) - screenWidth / 8) + player.x * 1.1), (((-canvas.height / 2) + screenHeight / 40) + player.y * 1.1), playerRadius / 5, 0, Math.PI * 2, false);
	ctx.fillStyle = playerOutlineColor;
	ctx.fill();
	ctx.closePath();
	...
	for (let i = 1; i < enemysLoaded + 1; i++) {
		if (enemyAlive[i]) {
			ctx.beginPath();
			ctx.arc((((canvas.width / 2) - screenWidth / 8) + player.x + enemyX[i] / 10), (((-canvas.height / 2) + screenHeight / 40) + player.y + enemyY[i] / 10), enemyRadius / 5, 0, Math.PI * 2, false);
			ctx.fillStyle = enemyOutlineColor;
			ctx.fill();
			ctx.closePath();
		}
